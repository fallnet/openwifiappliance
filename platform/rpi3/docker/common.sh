...
bootstrap(){
	...
	# fetch
#	local ROOTFS_DOWNLOAD_DIR="$2".download
#	if [ ! -d ${ROOTFS_DOWNLOAD_DIR} ]; then
#		debootstrap --download-only --components=main,contrib,non-free \
#			--arch armhf \
#			--foreign \
#			--no-check-gpg \
#			$1 ${ROOTFS_DOWNLOAD_DIR}.tmp $3
#		mv ${ROOTFS_DOWNLOAD_DIR}.tmp ${ROOTFS_DOWNLOAD_DIR}
#		exit 0
#	fi
	# install
	${BOOTSTRAP_CMD} --components=main,contrib,non-free \
		...
		--no-check-gpg \
		$1 $2 $3
#		$1 ${ROOTFS_DOWNLOAD_DIR} $3 
#	mv ${ROOTFS_DOWNLOAD_DIR} $2
}
export -f bootstrap

copy_previous(){
	if [ ! -d "${PREV_ROOTFS_DIR}" ]; then
		echo "Previous stage rootfs not found. (${ROOTFS_DIR})"
		exit 1
	fi
	mkdir -p ${ROOTFS_DIR}
	rsync --progress --one-file-system -aHAX ${PREV_ROOTFS_DIR}/ ${ROOTFS_DIR}/
}
